...
import { criarAcomodacao } from '../../services/acomodacaoService';
import { type AcomodacaoFormData } from '../../types/acomodacao';
import { mapFormToCreatePayload } from '../../utils/acomodacaoMapper';
...
export default function AcomodacaoCreate() {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const created = await criarAcomodacao(payload)
            toast.success(`Acomodação "${created.nome}" cadastrada com sucesso!`)
            setFormData({ nome: '', camaCasal: 0, camaSolteiro: 0, climatizacao: true, garagem: 0, suite: 0 })
        } catch (err: any) {
            console.error(err)
            toast.error(err.message || 'Erro ao cadastrar acomodação.')
        }
    }
